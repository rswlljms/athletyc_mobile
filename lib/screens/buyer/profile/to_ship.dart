...
class ToShip extends StatefulWidget {
  @override
  ToShipState createState() => ToShipState();
}

class ToShipState extends State<ToShip> {
  List<dynamic> orders = [];
  ...
  Future<void> fetchToShip() async {
    final prefs = await SharedPreferences.getInstance();
    final email = prefs.getString('email') ?? '';

    final response = await http.get(AppConfig.show_to_ship(email));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: orders.isEmpty
        ? Center(
            child: Text(
              'No orders to ship at the moment',
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Image.network(
                      //'http://192.168.1.110:5000/${item['image_paths']}',
